...
// Function to compare two arrays with given accuracy
int compareArrays(long long *arr1, long long *arr2, int size, const char *label) {
    for (int i = 0; i < size; i++) {
        if (fabs(arr1[i] - arr2[i]) > ACCURACY) {
            printf("Difference found in %s at index %d: %lld vs %lld\n", label, i, arr1[i], arr2[i]);
            return 0; // Return 0 if there is a significant difference
        }
    }
    printf("%s arrays match within accuracy %.2f.\n", label, ACCURACY);
    return 1; // Return 1 if all values match within accuracy
}

// Main comparison function
int compareResults(long long *DD, long long *DD_PAR, long long *RRS, long long *RRS_PAR,
                   long long *DRS, long long *DRS_PAR, int nbins) {
    int result = 1;
    
    result &= compareArrays(DD, DD_PAR, nbins, "DD");
    result &= compareArrays(RRS, RRS_PAR, nbins, "RRS");
    result &= compareArrays(DRS, DRS_PAR, nbins, "DRS");
...
int main( int argc, char **argv )
{
  ...
  float *binb, w;
  long long *DD, *RRS, *DRS, *DD_PAR, *RRS_PAR, *DRS_PAR;
  size_t memsize;
  ...
  // memory for DD
  DD_PAR = (long long*)malloc(memsize);
  if (DD_PAR == NULL)
    {
      fprintf(stderr, "Unable to allocate memory\n");
      exit(-1);
    }
  bzero(DD_PAR, memsize);
    
  // memory for RR
  RRS_PAR = (long long*)malloc(memsize);
  if (RRS_PAR == NULL)
    {
      fprintf(stderr, "Unable to allocate memory\n");
      exit(-1);
    }
  bzero(RRS_PAR, memsize);
    
  // memory for DR
  DRS_PAR = (long long*)malloc(memsize);
  if (DRS_PAR == NULL)
    {
      fprintf(stderr, "Unable to allocate memory\n");
      exit(-1);
    }
  bzero(DRS_PAR, memsize);
...
#pragma omp parallel
  ...
  for (rf = 0; rf < args.random_count; rf++)
    {
      ...
      // compute RR
      doComputeParallel(random, npr, NULL, 0, 1, RRS_PAR, nbins, binb);

      // compute DR
      doComputeParallel(data, npd, random, npr, 0, DRS_PAR, nbins, binb);
    }
  }
  }
  ...
  compareResults(DD, DD_PAR, RRS, RRS_PAR, DRS, DRS_PAR, nbins);
	      ...
	      int nbins, float *binb)
{
  ...
  {
	long long *priv_array = (long long*)calloc(nbins + 2, sizeof(long long));;
  #pragma omp for 
  for (i = 0; i < ((doSelf) ? n1-1 : n1); i++)
    {
		//long long priv_array[ARR_SIZE] = {};
	  const register float xi = data1[i].x;
      const register float yi = data1[i].y;
      const register float zi = data1[i].z;
	  ...
	  if (dot >= binb[min]) 
	    {
	      priv_array[min] += 1; /*k = min;*/ 
	    }
	  else if (dot < binb[max]) 
	    { 
	      priv_array[max+1] += 1; /*k = max+1;*/ 
	    }
	  else 
	    { 
	      priv_array[max] += 1; /*k = max;*/ 
	    }
        }
    }
